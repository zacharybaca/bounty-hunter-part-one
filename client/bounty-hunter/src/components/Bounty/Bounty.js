...
export default function Bounty(props) {
    ...
    return (
        <div id="bounty-container">
            {!showForm ? <>
                <h1 id="name-heading"><span className="heading">Name:</span> {props.firstName[0].toUpperCase() + props.firstName.slice(1)} {props.lastName[0].toUpperCase() + props.lastName.slice(1)}</h1>
                <h3><span className="heading">Living:</span> {props.living ? "✅" : "❌"}</h3>
                <h3><span className="heading">Amount:</span> ${props.bountyAmount}</h3>
                <h3><span className="heading">Type:</span> {props.type[0].toUpperCase() + props.type.slice(1)}</h3>
